...
export const getCrewById = async (req: Request, res: Response) => {
    try {
        ...
        if (isNaN(crew_id)) res.status(400).send({error: "ID must be a number"});
        ...
        if (!crew) res.status(404).send({error: "ID not found"});
...
export const createCrew = async (req: Request, res: Response) => {
    try {
      ...
      if (!media_id) res.status(400).send({ error: "Media ID is required" });
